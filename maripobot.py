...
PIPELINES = ("news", "video", "reflection", "timeline", "timeline_reflection",
             "music", "image")
...
def _default_pipeline_state() -> dict:
    return {p: {"done": False, "attempts": 0, "next_attempt_ts": 0.0,
                "last_error": None}
            for p in PIPELINES}
    ...
    while True:
        ...
        for item in response.feed:
            ...
            if item.reply and item.reply.parent:
                reply_to = {
                    "author": item.reply.parent.author.handle,
...
def interpret_bio(
    ...
    """Use LLM to extract concrete facts from a user's Bluesky bio."""
    ...
    try:
        ...
        if not result:
            return None
        result = result.strip()
        return None if result.upper().strip(".") == "NADA" else result
    except Exception as e:
...
def update_user_profile(
    ...
    """Extract new self-disclosed facts from a thread and append them to the user's profile."""
    profiles = json.loads(PROFILES_JSON_PATH.read_text(encoding="utf-8"))
    ...
    if not new_facts:
        print(f"update_user_profile devolvio None para {author_handle}, abortando")
        return

    new_facts = new_facts.strip()

    # Case-insensitive check, also handles "NADA." or "Nada\n"
    if new_facts.upper().strip(".") == "NADA":
        print(f"New facts: {new_facts} -> Nada para actualizar")
        return
    ...
    print(f"perfil de {author_handle} actualizado con: {new_facts}")
...
def maybe_update_lessons(
    ...
    try:
        ...
        new_lessons = response.choices[0].message.content
        debugger_choice(response)

        if not new_lessons or new_lessons.strip().upper() == "NADA":
            print("lecciones: sin cambios")
            return False

        new_lessons = new_lessons.strip()
        ...
        if "## Lecciones destiladas" in history:
            ...
            if next_section != -1:
                ...
                updated = history[:insert_pos] + "\n" + new_lessons + history[insert_pos:]
            else:
                updated = history + "\n" + new_lessons
        else:
        ...
        print(f"lecciones actualizadas: {new_lessons}")
        return True
...
def run_reflection_pipeline(client: Client, or_client: OpenAI) -> None:
    reflexion = reflect_on_history(or_client)
    if not reflexion:
    ...
    # Update lessons in history.md -- personality stays untouched
    maybe_update_lessons(or_client)
...
def handle_remember_command(
    ...
    """Process a /remember command from a user; persist to their profile."""
    ...
    result = response.choices[0].message.content
    if not result or result.strip().upper().strip(".") == "NADA":
        return False
    ...
    profile_path.write_text(updated, encoding="utf-8")
    print(f"/remember de {author_handle} guardado: {result.strip()}")
    return True
...
def run_timeline_pipeline(client: Client, or_client: OpenAI) -> None:
    ...
    # Learn from what was seen in the timeline today
    maybe_update_lessons(or_client)
...
def fetch_top_mealtime_video() -> dict | None:
    ...
    feed = feedparser.parse(f"https://www.reddit.com/r/{chosen}/top.rss?t=week")

    for entry in feed.entries:
        # RSS entries link to the reddit post; the YouTube URL lives inside the HTML body
...
def run_image_pipeline(client: Client, or_client: OpenAI) -> None:
    ...
    # Generate comment (optional -- post anyway if it fails)
    comment = generate_image_comment(or_client, image_data["title"], image_data["subreddit"])
    text = truncate_to_graphemes(comment) if comment else image_data["title"][:280]
...
def try_run_pipeline(name: str, state: dict, client: Client, or_client: OpenAI) -> None:
    """
    Attempt to run a pipeline if it's due. On failure, schedule an exponential
    backoff retry. Persists state on every attempt.
    """
    ...
    # Respect backoff between failed attempts
    if pstate["next_attempt_ts"] and time.time() < pstate["next_attempt_ts"]:
        return

    func = PIPELINE_FUNCS[name]
    try:
        print(f"[scheduler] corriendo {name} (intento {pstate['attempts'] + 1})")
        ...
        print(f"[scheduler] {name} fallo (intento {pstate['attempts']}): {e}. "
